    ...
    <Modal title="❓ 홀덤, 이렇게 하면 돼요" onClose={onClose}>
      <div className="help-body">
        ...
        <p>
          ...
          돌아가고, <b>D</b> 배지가 딜러 버튼이에요. <b>토너먼트 모드</b>에선 블라인드가
          6핸드마다 인상되니 마냥 버티면 칩이 말라가고, <b>캐시 게임 모드</b>에선 블라인드가
          고정되는 대신 파산한 봇 자리에 새 성향의 손님이 앉아요.
        </p>
        <h3>🎭 상대 유형 사전 — 누가 어떤 타입인지는 비밀!</h3>
        <p>
          새 게임을 시작할 때마다 봇들의 성향이 <b>비밀리에</b> 배정돼요. 같은 유형이 여러 명
          겹칠 수도 있으니 소거법은 통하지 않아요 — 직접 관찰해서 알아내보세요:
        </p>
        <ul>
          <li><b>매니악</b> — 베팅과 레이즈가 잦고 블러핑이 많아요. 좋은 패로 받아치세요.</li>
          <li><b>콜링 스테이션</b> — 웬만하면 콜해요. 블러핑이 안 통하니 진짜 패로만 베팅.</li>
          <li><b>함정형</b> — 조용하다가 갑자기 커지는 베팅은 강한 패일 확률이 높아요.</li>
          <li><b>바위</b> — 거의 안 치지만, 치면 진짜예요. 이 상대의 레이즈는 일단 믿으세요.</li>
          <li><b>균형형</b> — 교과서대로 치는 무난한 타입.</li>
          <li><b>GTO형</b> — 균형 기계. 블러프와 진짜를 수학 비율로 섞고 당신을 관찰하지도
            않아요. 뜯을 약점이 거의 없으니, 얘가 의심되면 기본기로만 정직하게 치세요.</li>
          <li><b>변신형(프로)</b> — 자기 이미지를 보고 기어를 바꿔요. 블러프를 들킨 직후엔
            타이트하게, 강한 패를 보여준 뒤엔 공격적으로. 아무리 봐도 유형이 안 잡히면 얘예요.</li>
        </ul>
        <ul>
          <li>가끔은 <b>유형과 다르게</b> 치기도 하니 확신은 금물!</li>
          <li>봇들도 <b>당신을 관찰</b>해요 — 매번 접으면 블러핑을 더 하고, 다 받아주면 진짜 패로만 공격해요.</li>
          <li>🎓 코치에게 물어보면 봇별 통계를 보고 함께 유형을 추리해줘요.</li>
        </ul>
        <h3>💡 초보자 팁</h3>
        <ul>
