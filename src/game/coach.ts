...
- 존재할 수 있는 유형: 매니악(공격·블러핑 잦음), 콜링 스테이션(웬만하면 콜), 함정형(강한 패를 숨겼다가 나중에 크게), 바위(초타이트, 베팅하면 거의 진짜), 균형형(교과서적), GTO형(수학적 균형 — 상대를 관찰하지 않고 블러프·밸류를 고정 비율로 섞으며 사이징 텔이 없어 착취가 거의 불가능. 이 유형이 의심되면 뜯으려 하지 말고 기본기대로 치는 게 답).
